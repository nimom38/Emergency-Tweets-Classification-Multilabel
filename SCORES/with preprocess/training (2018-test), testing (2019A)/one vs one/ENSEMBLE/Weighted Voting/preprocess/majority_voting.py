...
classifier = [
  ...
  # joblib.load("MultinomialNB_no_shuffle.pkl"),
  # joblib.load("SGD_no_shuffle.pkl"),
]
weights = [
  0.6,
  0.4,
...
with open("majority_voting.run", 'w') as output:
  ...
  for i in range(len(testLabelPredicted[0])):
    ...
    for j in range(len(testLabelPredicted)):
      assert len(testLabelPredicted[j][i]) == 24
      for k in range(len(testLabelPredicted[j][i])):
        temp_weights[k] += testLabelPredicted[j][i][k]*weights[j]
    yay = []
    for j in range(24):
      if temp_weights[j] >= threshold:
        yay.append(diction[categories[j]])
    if len(yay) == 0:
