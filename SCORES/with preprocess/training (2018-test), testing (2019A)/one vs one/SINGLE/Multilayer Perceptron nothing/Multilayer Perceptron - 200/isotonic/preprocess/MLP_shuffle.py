...
def main():
  ...
  classifier = Pipeline([
     ...
     ('clf', CalibratedClassifierCV(base_estimator = MLPClassifier(hidden_layer_sizes=(200,), random_state=1, max_iter=60, warm_start=True), method = 'isotonic'))])

  classifier.fit(X_train, Y_train)

  joblib.dump(classifier, "MLP_shuffle.pkl") 
  ...
  # with open("file_linearSVC(0.1).txt", 'w') as output:
  #   for row in arr:
